...
public:
    ...
    class Value : public std::variant<std::string, double, FormulaError> {
    public:
        using variant::variant; // Inherit the constructors of std::variant

        // Add a constructor that takes an int and initializes the variant as a double
        Value(int value) : variant(static_cast<double>(value)) {}
    };
