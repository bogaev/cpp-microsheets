...
class Expr {
public:
    using ValueGetter = std::function<const CellInterface* (const Position&)>;
    virtual ~Expr() = default;
    virtual void Print(std::ostream& out) const = 0;
    virtual void DoPrintFormula(std::ostream& out, ExprPrecedence precedence) const = 0;
    virtual double Evaluate(const ValueGetter&) const = 0;
...
public:
    ...
    double Evaluate(const ValueGetter& value_getter) const override {
        double left = lhs_->Evaluate(value_getter);
        double right = rhs_->Evaluate(value_getter);
        double result = 0.;
        switch (type_) {
        case Add:
            result = left + right;
...
public:
    ...
    double Evaluate(const ValueGetter& value_getter) const override {
        double operand = operand_->Evaluate(value_getter);
        switch (type_) {
        case UnaryPlus:
            return operand;
        case UnaryMinus:
        ...
        default:
            // have to do this because VC++ has a buggy warning
            // assert шёл в прекоде.
            // видимо, заход в данную ветку говорит о фатальной ошибке,
            // после которой программа должна упасть
            // assert(false);
            return static_cast<ExprPrecedence>(INT_MAX);
        }
    }
...
public:
    ...
    double Evaluate(const ValueGetter& value_getter) const override {
        // если позиция ячейки невалидна
        if (!cell_->IsValid()) {
            throw FormulaError(FormulaError::Category::Ref);
        }
        const CellInterface* cell = value_getter(*cell_);
        // если ячейка не существует - вернётся число 0.
        if (!cell) { 
            return 0.;
        }
        // если в ячейке уже есть ошибка - выбросить ошибку (распространить дальше)
        auto value = cell->GetValue();
        FormulaError* err = std::get_if<FormulaError>(&value);
        if (err) {
            throw (*err);
        }
        // если внутри число, то вернуть его
        double* num = std::get_if<double>(&value);
        if (num) { 
            return *num;
        }
        // если ячейка существует, но в ней пустая строка - вернуть число 0.
        std::string* empty_str = std::get_if<std::string>(&value);
        if (empty_str && empty_str->empty()) {
            return 0.;
        }
        // здесь подразумевается, что единственный вариант значения в этой
        // точке кода, который остаётся - это непустая строка (в нашей упрощённой реализации)
        // т.е. если в ячейке, на которую указал пользователь
        // где подразумевается формула, на самом деле лежит что-то не представимое как
        // число, то выбрасывается эта ошибка
        // других мест, где строка рассматривается как ошибка в коде нет
